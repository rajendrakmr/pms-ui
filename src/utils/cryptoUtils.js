...
export const decryptData = async (encryptedObj, password) => {
    const key = await getKeyFromPassword(password, new Uint8Array(encryptedObj.salt));
    const decrypted = await crypto.subtle.decrypt({
        name: "AES-GCM",
        iv: toArrayBuffer(new Uint8Array(encryptedObj.iv)), // ✅ FIX
    }, key, toArrayBuffer(Uint8Array.from(atob(encryptedObj.cipher), c => c.charCodeAt(0))));

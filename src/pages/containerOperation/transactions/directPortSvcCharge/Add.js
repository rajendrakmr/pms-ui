...
const Add = () => {
    ...
    const handleRowChange = useCallback(async (index, field, value) => {
        setFormData((prev) => {
    ...
    const checkRowForPayment = useCallback((row) => {
        ...
        setPaymentRecord((prev) => {
            const exists = prev.some((item) => item?.id?.chitNo === row?.id?.chitNo &&
                item?.id?.srlNo === row?.id?.srlNo);
            if (exists) {
                return prev.filter((item) => !(item?.id?.chitNo === row?.id?.chitNo &&
                    item?.id?.srlNo === row?.id?.srlNo));
            }
            return [...prev, row];
        });
    ...
    const handleCalcChange = useCallback((index, field, rawValue) => {
    ...
    const validateRow = (row, index) => {
        const itemErrors = {};
        if (!row.cfsDate)
            itemErrors.cfsDate = "CFS Date is required";
        if (!row.service)
            itemErrors.service = "Service is required";
        if (!row.from)
            itemErrors.from = "From date is required";
        if (!row.to)
            itemErrors.to = "To date is required";
    ...
    const saveRow = useCallback(async (index) => {
        ...
        setInserting((prev) => { return { ...prev, index: index, isInserting: true }; });
        ...
        try {
            ...
            const detail = response?.success && response?.success?.serviceDetails?.length
                ? response?.success?.serviceDetails.map((item) => {
            ...
            setInserting((prev) => { return { ...prev, index: null, isInserting: false }; });
        }
    }, [auth]);
    const handleRazorpayPayment = () => {
        const totalAmount = paymentRecord.reduce((sum, row) => sum + (row.totalVal || 0), 0);
        if (totalAmount <= 0) {
    ...
    return (_jsxs("div", { className: "_rkContentBorder container-fluid py-3", style: { border: "1px solid black", marginTop: "7px", marginBottom: "70px" }, children: [_jsx("div", { className: "d-flex justify-content-between align-items-center text-white px-3 py-1 mb-3 fw-bold", style: { backgroundColor: "#023e8a" }, children: _jsx("span", { style: { fontSize: "12px" }, children: "\uD83D\uDC49 DPE Service Charge >> Add" }) }), _jsxs("div", { className: "row", children: [_jsx(RowFormCheckField, { label: "Container No", name: "containerNo", inputValue: formData.containerNo, error: errors.containerNo, required: true, onChange: handleChange, click: () => onChangeSelect("container", formData.containerNo) }), _jsx(RowFormInputField, { label: "Admission Chit No", name: "adChitNo", isDefault: true, inputValue: formData.adChitNo, error: errors.adChitNo, onChange: handleChange }), _jsx(RowFormInputField, { label: "Admission Time", name: "adTime", isDefault: true, inputValue: formData.adTime, error: errors.adTime, onChange: handleChange }), _jsx(RowFormInputField, { label: "CH Agent Name", name: "chAgentName", isDefault: true, inputValue: formData.chAgentName, error: errors.chAgentName, onChange: handleChange }), _jsx(RowFormInputField, { label: "Shipping Bill No", name: "shipBillNo", isDefault: true, inputValue: formData.shipBillNo, error: errors.shipBillNo, onChange: handleChange }), _jsx(RowFormInputField, { type: "date", label: "Delivery Date (Tentative)", name: "delDateTentive", inputValue: formData.delDateTentive, error: errors.delDateTentive, onChange: handleChange }), _jsx(RowFormInputField, { label: "Delivery Date (Actual)", name: "delDateActual", isDefault: true, inputValue: formData.delDateActual, error: errors.delDateActual, onChange: handleChange })] }), _jsx("div", { className: "text-white px-3 mb-3 mt-2 fw-bold", style: { backgroundColor: "#023e8a" }, children: _jsx("span", { style: { fontSize: "12px" }, children: "\u27A4 Details" }) }), _jsx("div", { className: "row", children: _jsxs("div", { className: "col-12", children: [_jsx("div", { style: { overflowX: "auto" }, children: _jsxs("table", { className: "custom-table text-white", children: [_jsx("thead", { style: { backgroundColor: "#023e8a" }, children: _jsxs("tr", { children: [_jsx("th", { style: { minWidth: "140px" }, children: "Action" }), _jsxs("th", { style: { minWidth: "155px" }, children: ["CFS No", _jsx("span", { className: "text-danger", children: "*" })] }), _jsxs("th", { children: ["CFS Date", _jsx("span", { className: "text-danger", children: "*" })] }), _jsxs("th", { style: { minWidth: "200px" }, children: ["Service", _jsx("span", { className: "text-danger", children: "*" })] }), _jsxs("th", { children: ["From", _jsx("span", { className: "text-danger", children: "*" })] }), _jsxs("th", { children: ["To", _jsx("span", { className: "text-danger", children: "*" })] }), _jsx("th", { style: { minWidth: "120px" }, children: "Rate" }), _jsx("th", { style: { minWidth: "160px" }, children: "Amount" }), _jsx("th", { style: { minWidth: "110px" }, children: "CGST" }), _jsx("th", { style: { minWidth: "110px" }, children: "SGST" }), _jsx("th", { style: { minWidth: "110px" }, children: "Total GST" }), _jsx("th", { style: { minWidth: "160px" }, children: "Total" }), _jsx("th", { style: { minWidth: "160px" }, children: "Payment No" }), _jsx("th", { style: { minWidth: "120px" }, children: "Payment Date" }), _jsx("th", { style: { minWidth: "200px" }, children: "Remarks" })] }) }), _jsx("tbody", { children: formData?.details.map((row, index) => (_jsx(DpeTableRow, { row: row, index: index, services: services, errors: errors, deleteRow: deleteRow, saveRow: saveRow, inserting: inserting, handleRowChange: handleRowChange, paymentRecord: paymentRecord, checkRowForPayment: checkRowForPayment, handleCalcChange: handleCalcChange }, index))) })] }) }), _jsx("button", { type: "button", className: "btn btn-primary btn-sm mt-2 mr-4", onClick: addRow, style: {
                                borderRadius: "0px",
                                backgroundColor: "#023e8a",
                                color: "#fff"
                            }, children: "+ Add Row" })] }) }), _jsxs("div", { className: "d-flex gap-3 justify-content-end", children: [_jsx("button", { type: "button", disabled: submitting, className: "btn btn-sm btn-secondary custom-form-control", onClick: () => navigate("/editDpeServiceCharge"), children: "Back to Search Page" }), _jsxs("button", { type: "submit", onClick: handleRazorpayPayment, className: `btn btn-warning btn-sm px-4 custom-form-control position-relative ${submitting ? "loading" : ""}`, disabled: submitting || (!(paymentRecord.length > 0) || canPay), style: {
                            minWidth: "100px"
                        }, children: [submitting && _jsx("span", { className: "spinner-center" }), !submitting && _jsx("span", { className: "btn-text", children: "Payment through POS" })] }), _jsx("button", { type: "submit", className: "btn btn-sm  btn-dark custom-form-control ", children: "Print Payment" })] }), modal && _jsx(PopUpCheckBox, { isOpen: modal, onClose: () => setModal(false), itemsPerPage: 12, apiRequest: apiRequest, setFormData: setFormData, config: config }), confirmPaymentModal && _jsx(ConfirmPaymentModal, { amount: paymentRecord.reduce((sum, row) => sum + (row.totalVal || 0), 0), isOpen: confirmPaymentModal || processingPayment, processing: processingPayment, onConfirm: onConfirmPayment, onCancel: () => setConfirmPaymentModal(false) }), processingPayment && _jsx(ProcessingPayment, { isOpen: processingPayment, message: "Waiting for Payment" })] }));
};
export default Add;
